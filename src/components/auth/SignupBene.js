import { Box, Button, Grid, Link, TextField, Typography } from '@mui/material'
import React from 'react'
import { bold_name, btn_connect, df_jfs_ac, df_jfs_ac_fdc, link, textField } from '../../theme/CssMy'
import { useNavigate } from 'react-router'

export default function SignupBene() {
    const navigate = useNavigate()
    return (
        <Box sx={{ ...df_jfs_ac, height: '100%', padding: '0 15%' }}>
            <Box sx={{ ...df_jfs_ac_fdc, width: '100%' }}>
                <Grid container spacing={3}>
                    <Grid item md={12}>
                        <Typography variant='h5' sx={{ ...bold_name }}>Good to see you!</Typography>
                    </Grid>
                    <Grid item md={6}>
                        <TextField sx={textField} placeholder='Name' />
                    </Grid>
                    <Grid item md={6}>
                        <TextField sx={textField} placeholder='Aadhar number' />
                    </Grid>
                    <Grid item md={6}>
                        <TextField sx={textField} placeholder='PAN number' />
                    </Grid>
                    <Grid item md={6}>
                        <TextField sx={textField} placeholder='Phone number' />
                    </Grid>
                    <Grid item md={6}>
                        <TextField sx={textField} placeholder='Password' />
                    </Grid>
                    <Grid item md={6}>
                        <TextField sx={textField} placeholder='Confirm password' />
                    </Grid>
                    <Grid md={12} item sx={df_jfs_ac}>
                        Already have an account? &nbsp; <p style={link} onClick={() => navigate('/login')}> Login </p>
                    </Grid>
                    <Grid item md={12}>
                        <Button sx={btn_connect} onClick={() => navigate('/dashboard')} >Signup as beneficiary</Button>
                    </Grid>
                </Grid>
            </Box>
        </Box>
    )
}
